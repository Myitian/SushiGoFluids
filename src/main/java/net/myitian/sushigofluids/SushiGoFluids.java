...
public class SushiGoFluids {
    ...
    public void onFMLCommonSetup(FMLCommonSetupEvent event) {
        FLUID_ITEM_MAP.put(SushiContent.Items.SOY_SAUCE.get(), SOY_SAUCE.get());
        FLUID_ITEM_MAP.put(SushiContent.Items.WASABI_PASTE.get(), WASABI_PASTE.get());
    }
    ...
    public void onAttachingCapabilities(final AttachCapabilitiesEvent<ItemStack> event) {
        ...
        if (!(itemStack.getItem() instanceof AmountItem amountItem))
            return;
        VirtualFluid fluid = FLUID_ITEM_MAP.get(amountItem);
        if (fluid == null)
            return;
